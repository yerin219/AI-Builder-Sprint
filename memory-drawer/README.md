...
- 핵심 AI 흐름: Document Parse 전체 인식 → Solar 문서 유형 후보 → 사용자 유형 확인 → 영수증·티켓은 Information Extract로 필드 추출 / 손편지는 저장된 Document Parse 본문 재사용 → 사용자 확인 → 선택적 티켓 회상 지원
...
| 문서 유형 | 처리 방식 | 앞면 정보 |
| --- | --- | --- |
| 영수증 | 원본 이미지에 Information Extract를 독립 호출 | 날짜, 가게명 |
...
- 사용자가 확인한 정보만 표시합니다.
- 영수증과 티켓은 원본 사진을 표시하지 않고, 날짜·가게명·행사명·장소·좌석 등 확인된 추출값만 앞면에 표시합니다.
...
| 영수증·티켓 정보 추출 | Upstage Information Extract | 원본 이미지에서 날짜·가게명·행사명·장소·좌석 중 유형별 필드만 추출 |
...
| 제목 생성 | Upstage Solar LLM | 비어 있지 않은 사용자 답변만을 바탕으로 수정 가능한 제목 한 줄 생성 |
...
| 구분 | 앞면 | 뒷면 | AI 회상 질문 |
| --- | --- | --- | --- |
| 영수증 | 흰색 카드, 날짜, 가게명(원본 사진 미표시) | 동행인·날씨·기분, 사진 또는 자유 기록 | 사용하지 않음 |
...
AI는 종이에서 날짜·장소·글자를 찾아 입력 부담을 줄입니다. 티켓에서 사용자가 AI 질문 기능을 선택하면 서버에 미리 정의된 질문을 보여주고, Solar는 그 질문에 대한 사용자 답변만 바탕으로 수정 가능한 제목 한 줄을 제안합니다. Solar가 질문이나 별도의 한 줄 추억을 새로 만들지는 않습니다. 누구와 함께했는지, 어떤 순간이 마음에 남았는지, 그날 무엇을 느꼈는지는 사용자가 직접 기록합니다.
...
- Information Extract: 사용자가 문서 유형을 확정한 뒤 영수증과 티켓에만 호출합니다. Document Parse 결과가 아니라 보관된 원본 이미지에 독립적으로 호출해 영수증의 날짜·가게명과 티켓의 날짜·행사명·장소·좌석을 추출합니다. 손편지에는 호출하지 않습니다.
- Solar: 저장된 Document Parse 결과로 최초 문서 유형 후보를 생성하고, AI 질문 기능을 선택한 티켓의 세부 유형 후보를 생성합니다. 또한 서버의 고정 질문에 대한 사용자 답변만 바탕으로 수정 가능한 제목 한 줄을 생성합니다.

...
- 미확정 정책(사진 개수·용량, 글자 수 제한, 정렬 기준 등)은 AI가 임의로 결정하지 않고 팀 회의 또는 명세 확정 뒤 반영했습니다.
...
- 팀원 직접 결정: DIRECT와 AI_RECALL은 서로 다른 `draftId`를 사용하고, 질문 문구와 ID는 백엔드 응답을 사용했습니다. 같은 연도 카드 정렬은 오래된 추억 날짜순으로 확정했으며 AI 제목은 사용자가 수정·확정한 뒤 저장했습니다.
...
- 결과: 로그인부터 카드 저장, 연도별 서랍·상세 조회, 보호 이미지 조회까지의 흐름과 프론트 계약 테스트 27개를 확인했습니다.
...
- 팀원 직접 결정: 사진 확인 이미지는 기존 표시 영역의 1/5 크기로, 문서 유형 버튼은 영수증 → 티켓 → 손편지 순서의 세로 갈색 버튼으로 정했습니다. 모바일 화면의 여백·글자 크기와 PC 촬영 제한 안내 문구는 직접 조정했습니다.
- 검증: `npm test`, `npm run lint`, `npm run build`, `gradlew.bat test`, `git diff --check`, 모바일 브라우저에서 긴 텍스트·스크롤·카드 영역 넘침 여부 수동 확인
- 관련 기록: PR #51, #53, #55~#60

## 4. AI 활용 결과와 품질 기여

- API 명세와 프론트엔드·백엔드 계약을 반복 검토해 필드명, 상태 전이, 오류 코드, 인증 방식의 불일치를 줄였습니다.
- 이미지 분석 결과를 바로 저장하지 않고 사용자가 수정·확정하도록 해 AI 결과의 오인식 위험을 줄였습니다.
- 티켓·영수증·손편지의 긴 텍스트와 이미지가 카드 디자인을 벗어나지 않도록 화면을 보완했습니다.
- 카드 소유권 검증, 보호 이미지 조회, 저장 실패 시 파일 정리, 손편지 배경 제거 실패 시 텍스트 표시 등 예외 처리를 강화했습니다.
- 테스트·빌드·Git 검사와 실제 사용자 흐름 테스트를 통해 기능을 검증했습니다.

## 5. 상세 기록 위치

- 아래에는 팀원별 원본 AI 사용 기록과 세부 프롬프트·작업 내역을 보존합니다.
- 프로젝트 규칙과 에이전트 협업 방식은 `AGENTS.md`에서, API 계약과 오류 코드는 `docs/API_SPEC.md`에서 확인할 수 있습니다.

...
이 값은 손편지 앞면 확정 결과와 저장된 카드의 목록·상세 응답에 포함됩니다. 프론트엔드는 `BACKGROUND_REMOVED`이면서 `frontImageUrl`이 있는 경우에만 품질 검사를 통과한 투명 PNG를 표시합니다.
...
- 이미지 URL은 별도 서명 URL이 아닌 인증이 필요한 상대 API 경로이며, 요청할 때마다 Bearer 토큰으로 소유권을 검증합니다.
...
- 임시 기록은 생성 후 7일 동안 보관하며, 백엔드는 1시간 간격으로 만료된 미저장 임시 기록과 연결 파일을 정리합니다.
...
- 임시 기록은 7일 동안 보관하고 1시간 간격으로 만료 데이터를 정리합니다.
- 카드 앞면은 확정 데이터로 프론트엔드에서 렌더링하며, 손편지는 배경 제거 품질 검사를 통과한 경우에만 보호된 투명 PNG를 함께 표시합니다.
